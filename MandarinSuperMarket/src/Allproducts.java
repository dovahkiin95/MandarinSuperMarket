...
public class Allproducts {
	...
	public Allproducts() {
		...
		try {
			populate = ob.con
					.prepareStatement("SELECT Item,Price from Products");
			...
			while (r.next()) {
				...
				ComboBoxListener.buttons[i] = new JButton("" + product); // create
																			// button
																			// &
																			// add
																			// to
																			// array
				// buttons[i].addActionListener(this); //add an action listener
				// to the current button
				// other attributes you will set
				ComboBoxListener.buttons[i].setPreferredSize(new Dimension(190,
						30));
				CustomerWindow.buttonPanel.add(ComboBoxListener.buttons[i],
						CustomerWindow.constraint);
				ComboBoxListener.buttons[i]
						.addActionListener(new ActionListener() {
							...
							public void actionPerformed(ActionEvent event) {
								...
								if (count > 0)
									ProductPriceTable1.d.setVisible(false);

								CustomerWindow.textField_1.setText(product);
								CustomerWindow.textField_2.setText(String
										.valueOf(price));
								CustomerWindow.frame.revalidate();
								CustomerWindow.frame.repaint();
								new ProductPriceTable1(((JButton) event
										.getSource()).getText(), pi);
								count++;
