...
public class ComboBoxListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			populate = ob.con
					.prepareStatement("SELECT Item,Price from Products where D_num in (select D_no from Category where Department=?)");
			...
			while (r.next()) {
				...
				buttons[i] = new JButton("" + product); // create button & add
														// to array
				...
				buttons[i].addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent event) {
						...
						String temp = ((AbstractButton) event.getSource())
								.getText();
						...
						new ProductPriceTable1(((JButton) event.getSource())
								.getText(), pi);
				...
				CustomerWindow.buttonPanel.add(buttons[i],
